...
public class Operations {
    ...
    void animeListEmbed(MessageReceivedEvent event){
        ...
        StringBuilder desc = new StringBuilder("```css\nRank   " + String.format("%-22s", "Anime") + "Rating  Type  " +
                "Episodes  Watched\n```");
        ...
        for (int i = startRank; i < endRank; ++i)
        {
            if (i < listCopy.size())
            {
                ...
                if (local != null)
                {
                    if (i > 4)
                    {
                        int maxLengthName = (local.getName().length() < 22)?local.getName().length():22;
                        int maxLengthType = (local.getType().length() < 5)?local.getType().length():5;
                        descThree.append(i + 1)
                                .append(i > 8 ? ". " : ".  ")
                                .append(String.format("%-22s  ", local.getName().substring(0, maxLengthName)))
